...
describe('freePendingDocuments', () => {
  it('should free all the document that are pending from more than the given minutes', async () => {
    ...
    const updatedDocument0 = await documentRepository.findById(documents[0]._id);
    const updatedDocument1 = await documentRepository.findById(documents[1]._id);
    const updatedDocument2 = await documentRepository.findById(documents[2]._id);
    const updatedDocument3 = await documentRepository.findById(documents[3]._id);
    const updatedDocument4 = await documentRepository.findById(documents[4]._id);
