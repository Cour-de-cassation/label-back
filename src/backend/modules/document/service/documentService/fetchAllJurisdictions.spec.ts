...
describe('fetchAllJurisdictions', () => {
  it('should fetch all the distinc jurisdictions', async () => {
    ...
    const documents = ['Cour de cassation', "cour d'appel de Dijon", '', "cour d'appel de Bordeaux"].map(
      (jurisdiction) =>
    ...
    expect(jurisdictions.sort()).toEqual(
      ['Cour de cassation', "cour d'appel de Dijon", "cour d'appel de Bordeaux"].sort(),
    );
  });
});
